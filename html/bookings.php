...
try {
    ...
    $sql = "SELECT *, image AS image FROM travels WHERE itemName LIKE? AND vertrekdatum LIKE? AND accommodatie LIKE? AND vliegtuig LIKE? AND dagenaantal LIKE?";
    $params = [
        '%'. $bestemming. '%',
        '%'. $vertrekdatum. '%',
        '%'. $accommodatie. '%',
    ...
    if ($stmt->rowCount() > 0) {
        ...
        foreach ($results as $row) {
            ...
            echo "<img src='/webapp2/img/". htmlspecialchars($row["image"]?? ''). "' alt='". htmlspecialchars($row["itemName"]?? ''). "'>";
            echo "<div class='card-content'>";
            echo "</div>";
            echo "<p class='Destination'><a href='bookings.php?itemName=". urlencode($row["itemName"]?? '')."&bestemming=". urlencode($bestemming)."&vertrekdatum=". urlencode($vertrekdatum)."&accommodatie=". urlencode($accommodatie)."&vliegtuig=". urlencode($vliegtuig)."&dagenaantal=". urlencode($dagenaantal)."'>". htmlspecialchars($row["itemName"]?? ''). "</a></p>";
            ...
            if (isset($_GET['itemName']) && $_GET['itemName'] == $row["itemName"] && isset($row["ProductCode"])) {
                echo "<p class='info-reis'>". htmlspecialchars($row["bestemming"]?? ''). "</p>";
                echo "<p class='info-reis'>". htmlspecialchars($row["vertrekdatum"]?? ''). "</p>";
                echo "<p class='info-reis'>". htmlspecialchars($row["accommodatie"]?? ''). "</p>";
                echo "<p class='info-reis'>". htmlspecialchars($row["vliegtuig"]?? ''). "</p>";
                echo "<p class='info-reis'>". htmlspecialchars($row["dagenaantal"]?? ''). " dagen</p>";
                echo "<form action='book_query.php' method='POST' class='book-button'>
